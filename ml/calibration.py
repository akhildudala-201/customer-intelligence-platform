...
CONFIG = {
    ...
    "OUTPUT_MODEL_DIR": str(PROJECT_ROOT / "app" / "ml" / "outputs" / "models") + "/",
    "OUTPUT_REPORT_DIR": str(PROJECT_ROOT / "app" / "ml" / "outputs" / "reports") + "/",
...
def load_recommended_tune_metric(
    ...
    hardcoding a choice:
      1. Prefer app/ml/outputs/reports/best_threshold_comparison_val.csv -- pick the
      ...
      2. Fall back to app/ml/outputs/reports/best_thresholds_LightGBM_val.csv -- pick
...
def load_latest_lightgbm_model() -> Optional[Any]:
    """Load the most recently saved LightGBM TRAINING artifact from app/ml/outputs/models/, if any.
    ...
    if not candidates:
        log_message(
            "No saved LightGBM training artifact found in app/ml/outputs/models/ "
...
def main():
    ...
    if lgbm_model is None:
        log_message(
            "No LightGBM training artifact found in app/ml/outputs/models/. "
