...
for candidate in (str(PROJECT_ROOT), str(APP_ROOT), str(FEATURES_DIR)):
    if candidate not in sys.path:
        sys.path.insert(0, candidate)

try:
    from app.Features.build_churn_label import build_labels
...
def save_combined_dataset(dataset: pd.DataFrame):
    ...
    print(f"\nCombined database table replaced: {TABLE_NAME}")
