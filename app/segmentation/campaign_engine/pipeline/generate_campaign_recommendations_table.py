...
def generate_campaign_recommendations_table() -> pd.DataFrame:
    """
    Builds the merged campaign-input base, runs it through the Campaign
    Recommendation Engine, narrows to OUTPUT_COLUMNS, and writes it to
    `customer_campaign_recommendations`.

    Returns the written DataFrame so callers (tests, notebooks, a
    scheduler) can inspect what was persisted without a second DB read
    — same convention as generate_clv_table.py's generate_clv_table().
    """
...
def _print_summary(output: pd.DataFrame) -> None:
    """Separate from generate_campaign_recommendations_table() itself so
    tests/other callers that just want the DataFrame aren't forced to
    also print — same convention as generate_clv_table.py."""
