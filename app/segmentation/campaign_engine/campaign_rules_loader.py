...
class CampaignRulesConfig:
    ...
    def lookup(self, risk_tier: str, segment: str, value_tier: str) -> tuple[CampaignRule, bool]:
        """Returns (rule, matched_explicit_rule). matched_explicit_rule is
        False when the (risk_tier, segment, value_tier) combination fell
        through to default_rule — recommender.py uses this to report how
        many customers hit an explicit rule vs. the catch-all, per the
        config's own "Any combination NOT listed falls back to
        default_rule" convention."""
...
def load_campaign_rules_config(config_path: Path = CONFIG_PATH) -> CampaignRulesConfig:
    """Cached — same convention as risk_tier/classifier.py's
    load_risk_tier_config(): call load_campaign_rules_config.cache_clear()
    to pick up an edit to the YAML within the same process."""
