...
def _load_env_file():
    ...
    for parent in [search_root.parent, *search_root.parents]:
        env_path = parent / ".env"
        if env_path.exists():
            try:
...
try:
    from app.Database.cleaning import clean_dataframe
except ImportError:
...
def load_table(table_name, valid_keys):
        ...
        # Geolocation can be large; process it in chunks to keep memory usage bounded.
        ...
        # Track surviving parent IDs so child tables can drop orphaned records before insert.
        surviving_keys = (
            set()
            if table_name in PRIMARY_KEYS
            else None
