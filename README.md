...
`retention_rate (%)`, `repeat_purchase_rate (%)`, `average_clv`, and
`generated_date`. `average_clv` is total revenue generated by the cohort divided
by the total number of customers in that cohort, matching the cohort
specification. It is repeated for each relative-month row for that cohort.
