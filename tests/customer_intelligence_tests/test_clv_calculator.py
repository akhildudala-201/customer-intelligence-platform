...
Unit tests for app/segmentation/customerintelligence/clv_calculator.py.

Every test in ComputeCustomerClvTests / ValidationTests passes `features`
explicitly into compute_customer_clv(), so nothing here touches the real
database — that's exactly why compute_customer_clv() accepts an optional
`features` argument in the first place (see its docstring). Only
GetCustomerFeaturesTests exercises the DB-reading path, and it does so
with pd.read_sql_table mocked out, never a real connection.

Run with:
    python -m unittest app.segmentation.customerintelligence.test_clv_calculator -v
or, from the same folder:
    python -m unittest test_clv_calculator -v
"""
...
class ComputeCustomerClvTests(unittest.TestCase):
    ...
    def test_lifespan_and_frequency_do_not_share_a_denominator(self):
        """Guards against the exact bug the module docstring warns about:
        if lifespan were each customer's own tenure instead of a shared
        cohort constant, it would equal purchase_frequency's denominator
        and CLV would collapse to AOV * frequency (i.e. revenue-to-date)
        for every repeat customer. Confirm that does NOT happen."""
        result = compute_customer_clv(features=self.features.copy())
...
class GetCustomerFeaturesTests(unittest.TestCase):
    ...
    @patch("app.segmentation.customer_intelligence.clv_calculator.pd.read_sql_table")
